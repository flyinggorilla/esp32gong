 ...
 */
char* WebServerOld::createInfoJson() {
	/*if (this->printedJson) {
	 free(this->printedJson);
...
const char* WebServerOld::mgEventToString(int event) {
	switch (event) {
	case MG_EV_CONNECT:
...
char* WebServerOld::mgStrToStr(struct mg_str mgStr) {
...
void HttpUploadHandler(struct mg_connection *nc, int event, void *p) {
	//WebServerOld* ws = (WebServerOld*) nc->user_data;
	//TODO WebServerOld structur maybe not neede; instead create a Spiffs object for handling upload;
...
void HttpRequestHandler(struct mg_connection *nc, int ev, void *evData) {
	...
	case MG_EV_HTTP_REQUEST: {
		...
		char *uri = WebServerOld::mgStrToStr(message->uri);
		char* query = WebServerOld::mgStrToStr(message->query_string);
		...
		if (strcmp(uri, "/info") == 0) {
			char* jsontext = WebServerOld::createInfoJson();
...
void WebServerOldTask(void* user_data) {
	WebServerOld* ws = (WebServerOld*) user_data;
	...
	while (true) {
		...
		ESP_LOGI(LOGTAG, "Binding WebServerOld to port 80");
		while (!nc) {
			nc = mg_bind_opt(&mgr, ":80", HttpRequestHandler, opts);
			if (!nc) {
				ESP_LOGW(LOGTAG, "Binding WebServerOld failed, retrying in 5 seconds....");
				vTaskDelay(5000 / portTICK_PERIOD_MS);
			}
		}
		ESP_LOGI(LOGTAG, "WebServerOld successfully bound");
...
void WebServerOld::start() {
	//gpio_pad_select_gpio(LED_BUILTIN);
	ESP_LOGI(LOGTAG, "WebServerOld starting...");

	// Set the GPIO as a push/pull output
	//gpio_set_direction(LED_BUILTIN, GPIO_MODE_OUTPUT);
	//xTaskCreate(&generate_json, "json", 2048, NULL, 5, NULL);
	//xTaskCreatePinnedToCore(&WebServerOldTask, "WebServerOldTask", 20000, NULL, 5, NULL,0);
	xTaskCreate(&WebServerOldTask, "WebServerOldTask", 4096, this, 5, NULL);
